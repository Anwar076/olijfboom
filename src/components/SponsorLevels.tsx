    ...
    <section className="py-20 px-4">
      <div className="container mx-auto max-w-6xl">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold mb-4 title-gradient">Sponsorniveaus</h2>
          <p className="text-xl text-slate-700">Kies jouw niveau van ondersteuning</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {levels.map((level, index) => (
            <div
              key={index}
              className="bg-white/80 rounded-2xl p-6 border border-slate-200 hover:border-gold transition-colors backdrop-blur-sm"
            >
              <div className="text-3xl font-bold text-gold mb-2">{level.name}</div>
              <div className="text-xl font-semibold text-slate-700 mb-3">{level.amount}</div>
