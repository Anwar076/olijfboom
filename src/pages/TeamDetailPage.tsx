    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="pt-32 pb-20 px-4">
        <div className="container mx-auto max-w-4xl">
          <button
            onClick={() => navigate('/')}
            className="text-slate-600 hover:text-gold transition-colors mb-8"
          >
            ← Terug naar homepage
          </button>
          ...
          <div className="bg-white/80 rounded-3xl p-8 md:p-12 border border-slate-200 backdrop-blur-sm">
            <div className="flex items-start justify-between mb-4">
              <h1 className="text-4xl md:text-5xl font-bold title-gradient">{team.name}</h1>
              {/* Lampje indicator */}
            ...
            {team.description && (
              <p className="text-slate-700 mb-8 text-lg">{team.description}</p>
            )}

            <div className="mb-8">
              <div className="flex justify-between items-center mb-4">
                <span className="text-slate-600">Totaal opgehaald</span>
            ...
            <div className="border-t border-slate-300 pt-8">
              <h2 className="text-2xl font-bold mb-6 title-gradient">Teamleden</h2>
              {team.members.length === 0 ? (
                <p className="text-slate-600">Nog geen teamleden</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {team.members.map((member, index) => (
                    <div
                      key={index}
                      className="bg-white/80 rounded-lg p-4 border border-slate-300"
                    >
                      <div className="text-slate-900 font-medium">{member}</div>
                    </div>
                  ))}
                </div>
              )}
